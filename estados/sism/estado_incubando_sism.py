...
class EstadoIncubandoSISM(EstadoNodo):
    ...
    def transicionar(self, nodo, grafo):
        ...
        if self.tiempo_incubacion == 0:
            ...
            if random.random() < prob_de_infeccion_grave:
                estado_actual = EstadoInfectadoGraveSISM(tiempo_infeccion=grafo.graph['tiempo_inf_grave'])
            else:
                estado_actual = EstadoInfectadoMildSISM(tiempo_infeccion=grafo.graph['tiempo_inf_mild'])
